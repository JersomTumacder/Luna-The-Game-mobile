...
function update(time) {
  ...
  if (!gameStarted) {
    return; // Exit update loop if game hasn't started
  }
...
function handleStart() {
  if (gameStarted) return; // Prevent starting again if game is already started
  gameStarted = true;
  lastTime = null;
  speedScale = 1;
  score = 0;
...
function setupEventListeners() {
  ...
  // Event listener for starting the game (mousedown, touchstart, click)
  document.addEventListener("mousedown", handleStart);
  document.addEventListener("touchstart", handleStart);
  document.addEventListener("click", handleStart);

  // Event listener for jumping (mousedown, touchstart)
  document.addEventListener("mousedown", () => {
    if (gameStarted) {
      jump(); // Call jump() function when mouse is clicked (only if game is started)
    }
  });
  document.addEventListener("touchstart", () => {
    if (gameStarted) {
      jump(); // Call jump() function when touch is started (only if game is started)
    }
  });
}
